...
while(True):
    ...
    # Once per day (currently set for 6:00am local time, or when script
    # is first run, if after 6:00am), run forecast and sudoku scripts.
    l = time.localtime()
    if (60 * l.tm_hour + l.tm_min) > (60 * 6):
        if dailyFlag == False:
            daily()
            dailyFlag = True
    else:
